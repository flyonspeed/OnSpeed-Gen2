...
#define VERSION           "v3.0.3"  // last modified 3/15/2020 by Lenny, updated AI-Pitch network parameters
                          //"v3.0.2"  // last modified 3/10/2020 by Lenny, fixed pitchBias, confirm flap position delete, fixed Glimit audio
                          //"v3.0.1"  // last modified 3/8/2020 by Lenny, added neural network based pitch calculation
...
int accMedianSmoothing=31; // median filter window for accelerations, for calculating pitch
int accSmoothing=20; // accelerometer smoothing, Gaussian
int accLatSmoothing=50; // lateral G needs more smoothing;
int gyroSmoothing=20; // gyro smoothing, Gaussian
int pitchSmoothing=30; // accPitch smoothing, Gaussian
...
GaussianAverage aLatAvg = GaussianAverage(accLatSmoothing);
...
volatile unsigned long lastGyroReadTime=micros(); // gyro elapsed time since last read
volatile unsigned long gyroDT=0;
    ...
    if (dataSource=="SENSORS" && sdAvailable && sdLogging)sdAvailable=SD.begin(SdioConfig(FIFO_SDIO));
        ...
        if (sdLogging)
            ...
            if (SensorFile) {
                            ...
                            #ifdef IMU
                            SensorFile.print(",VerticalG,LateralG,ForwardG,RollRate,PitchRate,YawRate,Pitch");
                            #endif        
...
aVertAvg+=getAccelForAxis(verticalGloadAxis); aVert=aVertAvg.process().mean;
aLatAvg+=getAccelForAxis(lateralGloadAxis); aLat=aLatAvg.process().mean;
aFwdAvg+=getAccelForAxis(forwardGloadAxis); aFwd=aFwdAvg.process().mean;
...
// AI-AHRS calculations
//unsigned long netstarttime=micros();
data.value[0]=getAccelForAxis(verticalGloadAxis);
data.value[1]=getAccelForAxis(lateralGloadAxis);
data.value[2]=getAccelForAxis(forwardGloadAxis);
data.value[3]=getGyroForAxis(pitchGyroAxis);
data.value[4]=getGyroForAxis(rollGyroAxis);
data.value[5]=getGyroForAxis(yawGyroAxis);
data.value[6]=Pstatic;
data.value[7]=ASI;

predictedPitch=classifier.Predict(data);
accPitchAvg+=predictedPitch[0];
accPitch=accPitchAvg.process().mean+pitchBias; // get Gaussian smoothed pitch and add bias (degrees).
//Serial.println(micros()-netstarttime);
#endif   
...
if (sdLogging)
        {
          ...
          #ifdef IMU
          charsAdded+= sprintf(logLine+charsAdded, ",%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.2f",getAccelForAxis(verticalGloadAxis),getAccelForAxis(lateralGloadAxis),getAccelForAxis(forwardGloadAxis),getGyroForAxis(rollGyroAxis),getGyroForAxis(pitchGyroAxis),getGyroForAxis(yawGyroAxis),accPitch);
          #endif
          if (readBoom)
            { 
             ...
             return;
                ...
                // end of log line, parse it              
                  ...
                  if (PfwdPascal>0)
                      {
                      ASI=sqrt(2*PfwdPascal/1.225)* 1.94384; // knots // physics based calculation
                      ASI=curveCalc(ASI,casCurve); // apply correction curve
                      }
                      else ASI=0;                                                        
                  #ifdef BARO
                  Palt=valueArray[6].toFloat();
                  Pstatic=valueArray[5].toFloat();
                  //Palt=145366*(1-pow(Pstatic/1013.2,0.190284));
                  #endif

                  #ifdef IMU
                  aVert=stringToFloat(valueArray[11]);
                  aLat=stringToFloat(valueArray[12]);
                  aFwd=stringToFloat(valueArray[13]);
                  gRoll=stringToFloat(valueArray[14]);
                  gPitch=stringToFloat(valueArray[15]);
                  gYaw=stringToFloat(valueArray[16]);
...
void readAccelGyro()
{
  ...
  gyroDT=micros()-lastGyroReadTime;
...
void SendDisplayData()
{
 ...
 char crc_buffer[128];
 byte CRC=0;
 float verticalGload=sqrt(abs(aVert*aVert+aLat*aLat+aFwd*aFwd));
